 ...
 * closed.
 */
public class FileSensorLog extends BaseSensorLog {
    ...
    private final String TAG = "TextFileLOG";
     ...
     */
    public FileSensorLog(File file) throws IOException {
        this(file, "");
    }
     ...
     */
    public FileSensorLog(File file, String notes) throws IOException {
     ...
     */
    public FileSensorLog(File file, String notes, boolean resume) throws IOException {
        this.file = file;
     ...
     *         the timestamp to put into the file instead of the real collection
     *         time, or -1 to restore normal timestamp logging.
     */
    static void setTimestampForTest(long overrideTimestamp) {
        FileSensorLog.overrideTimestamp = overrideTimestamp;
    }
    ...
    @Override
    public void logUSBDeviceData(String sensor, float length) {
        float[] floats = new float[1];
        floats[0] = length;
        Common.database.insert_Accel_data(sensor, System.currentTimeMillis(), floats, true);
    }
    ...
    protected synchronized void logSensorEvent(long absoluteTimeNanos, SensorEvent event) {
        ...
        Common.database.insert_Accel_data(Common.device_name+getSensorName(event.sensor.getType()),absoluteTimeNanos,values, false);
    }
    ...
    private void writeLocationLine(long absoluteTimeNanos, String key, Location loc) {
        ...
        if (false) {
            ...
            Log.i("FileSensorLog","writeLocationLine : I AM HERE");
            float[] coordinates = new float[2];
            coordinates[0] = latE7;
            coordinates[1] = lngE7;
            Common.database.insert_Accel_data(Common.device_name+"location", System.currentTimeMillis(), coordinates, false);
        }
